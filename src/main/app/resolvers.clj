...
(def people-table
  ...
  (atom
    {1 {:person/id 1 :person/name "Sally" :person/age 32}
     ...
     5 {:person/id 5 :person/name "Will" :person/age 41}}))
...
(pc/defresolver everyone-resolver [env input]
  {::pc/output [{:everyone [:person/id]}]}
  {:everyone (mapv (fn [id] {:person/id id}) (keys @people-table))})

(pc/defresolver qod [_ _]
  {::pc/output [:rest/qod]}
  (let [qod (-> @(http/get "https://quotes.rest/quote/random?language=en")
                :body
                bs/to-string
                json/read-str
                (get-in ["contents" "quotes" 0 "quote"])
                )]
    {:rest/qod qod}))

(pc/defresolver me-resolver [env input]
  {::pc/output [:me [:id]]}
  {:me {:id (get-in env [:request :identity :sub])} })

(def resolvers [qod me-resolver everyone-resolver person-resolver list-resolver friends-resolver enemies-resolver])
